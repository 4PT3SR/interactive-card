...
export const validationSchema = Yup.object().shape({
    ...
    cardNumber:Yup.number().typeError('Must be a number').required("Can't be blank!").min(16,'Atleast 16 digits').max(16,'At most 16 digits'),
    ...
    cvv:Yup.number().typeError('Must be a number').required("Can't be blank!").min(3, 'Atleast 3 digits'),
})
